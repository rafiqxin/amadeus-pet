 ...
 *
 * These are not committed: the renderer is useless without them, but they are
 * the author's own assets hosted alongside the Android app, and CI downloads
 * them the same way (see .github/workflows/build-android-apk.yml).
 *
 *     node tools/fetch-voices.mjs
 *     node tools/fetch-voices.mjs --verify-only
 *
 * Idempotent: existing non-empty files are left alone.
 */
